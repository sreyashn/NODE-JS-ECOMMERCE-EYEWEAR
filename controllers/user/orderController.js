...
const {calculateDiscountedTotal,calculateProductTotal,calculateSubtotal}=require("../../config/cartSum")
...
if (userData) {
 ...
 if (order.coupon) { 
    if (couponData) {
        ...
        // Calculate coupon amount based on the coupon percentage and order subtotal
        couponAmount = (couponPercentage / 100) * order.subtotal;
    }
  }
  console.log(couponData,"couponData")
  console.log("couponAmount:", couponAmount);
  console.log("couponPercentage:", couponPercentage);

    res.render("users/order",{ userData, order, totalPages,
      currentPage: page, couponPercentage, couponAmount  });
...
//         const couponAmount = await Coupon.findOne({coupon : couponCode})
//         console.log("////",couponAmount);
//       res.render("users/orderDetails",{userData, order,couponPercentage, couponAmount });
//     } catch (error) {
//       console.log(error.message);
//     }
//   };
...
const loadOrderHistory = async (req, res) => {
  try {
      ...
      // Initialize couponPercentage and couponAmount
      let couponPercentage = 0;
      let couponAmount = 0;
      ...
      // Check if the order has a coupon code
      if (order.coupon) {
          // Find the corresponding coupon document
          const coupon = await Coupon.findOne({ code: order.coupon });
          
          // If the coupon document is found, calculate coupon percentage and amount
          if (coupon) {
              couponPercentage = coupon.discount;
              couponAmount = (couponPercentage / 100) * order.totalAmount;
          }
      }
