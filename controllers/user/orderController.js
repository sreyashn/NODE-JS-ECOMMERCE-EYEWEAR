...
const loadOrderHistory = async (req, res) => {
  ...
  const orderCancel = async (req, res) => {
    ...
    const returnData = async (req, res) => {
      console.log("entered",returnData);
      // const orderId = req.query.id;
      ...
      if (product && product.product) {
        ...
        if (walletData) {
          ...
          walletData.walletBalance +=(product.price *  item.quantity)- (product.price *  item.quantity)*(couponData.discount)/100;
       
          walletData.transaction.push({
            type: "credit",
            amount:(product.price *  item.quantity)- (product.price *  item.quantity)*(couponData.discount)/100,
          });
          ...
          const wallet = new Wallet({
            user: user_id,
            transaction:[{type:"credit",amount: (product.price *  item.quantity)- (product.price * item.quantity)*(couponData.discount)/100}],
            walletBalance:  (product.price *  item.quantity)- (product.price * item.quantity)*(couponData.discount)/100
        });
        ...
        totalAmount =totalAmount -(product.price *  item.quantity)- (product.price *  item.quantity)*(couponData.discount)/100;
